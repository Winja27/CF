...
def cal_cosine(ratings1, ratings2):
    similarity = 0
    numerator = 0
    denominator1 = 0
    denominator2 = 0
    for i in range(len(ratings1)):
        ...
        denominator1 += ratings1[i] ** 2
        denominator2 += ratings2[i] ** 2
        ...
        if len(items_by_u) > 0 and len(items_by_v) > 0:
            ...
            if len(intersected) > 0:
                ...
                if ratings_u != None and ratings_v != None:
                    s = cal_cosine(ratings_u, ratings_v)
...
def visualization(df_uir, df_prediction):
    ...
    for a, b in dict_uir.items():
        for c in list(b.keys()):
            if math.isnan(b[c]):
                del b[c]
    mae_all = {}
    data = [[5, 0.709262500098963, 'Cosine']]
    ...
    for i in mae_all:
        ...
        df_row = pd.DataFrame(
            {'Neighbors': [i], 'MAE': [mae], 'Algorithm': ['Cosine']})
        print('Neighbors', i, 'MAE', mae, 'Algorithm', 'Cosine')
