...
def calculateMae(observation_dict_cos):
    mae_all = {}
    # 初始化MAE结果的DataFrame
    mae_df = pd.DataFrame(columns=['Neighbors', 'MAE', 'Algorithm'])
    # 遍历不同的邻居数量
    for k in observation_dict_cos:
        # 遍历每个用户-物品对的预测结果
         for kv in observation_dict_cos[k]:
            predicted = observation_dict_cos[k][kv][0]
            actual = observation_dict_cos[k][kv][1]
            # 计算预测误差
    ...
    for i in mae_all:
        ...
        df_row = pd.DataFrame({'Neighbors': [i], 'MAE': [mae], 'Algorithm': ['Cosine']})
        mae_df = pd.concat([mae_df, df_row], ignore_index=True)
    # 返回最终的MAE结果DataFrame
    return mae_df

# 计算误差
def calculateRmse(observation_dict_cos):
    # 存储不同邻居数量的MAE值
    rmse_all = {}
    # 初始化MAE结果的DataFrame
    ...
    for i in rmse_all:
        ...
        # 将结果添加到MAE结果的DataFrame中
        df_row = pd.DataFrame({'Neighbors': [i], 'RMSE': [rmse], 'Algorithm': ['Cosine']})
...
def processOverall(observation_dict,ave_rating):
    ...
    for k in observation_dict:
        ...
        p_row = pd.DataFrame({'Neighbors': [k], 'Precision': [precision], 'Algorithm': ['Cosine']})
        r_row = pd.DataFrame({'Neighbors': [k], 'Recall': [recall], 'Algorithm': ['Cosine']})
        a_row = pd.DataFrame({'Neighbors': [k], 'Accuracy': [accuracy], 'Algorithm': ['Cosine']})
        f_row = pd.DataFrame({'Neighbors': [k], 'F1': [f1], 'Algorithm': ['Cosine']})
...
def processNDCG(observation_dict,ave_rating,dict_uir):
    ...
    for k in observation_dict:
        ...
        n_row = pd.DataFrame({'Neighbors': [k], 'NDCG': [NDCG_result[k]], 'Algorithm': ['Cosine']})
...
def processHLU(observation_dict):
    ...
    for k in observation_dict:
        ...
        h_row = pd.DataFrame({'Neighbors': [k], 'HLU': [HLU_result[k]], 'Algorithm': ['Cosine']})
